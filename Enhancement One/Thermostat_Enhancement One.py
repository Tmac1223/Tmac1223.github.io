...
# with the GPIO interface on the Raspberry Pi board and work with the 16x2 LCD display
...
# lights to their own thread so that more work can be done at the same time
...
# creates a new file for temps to be logged
class temperatureLog():
    def log_temp_file():
        # gets the temp
        temp_f = floor(tsm.getFahrenheit())
        # gets the time in the format of year, month, day, hour, min, sec
        time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        # creates a entry to the log file
        log_entry = f"{time}, {temp_f}F\n"
        
        # add data to the file while its open
        with open("tempLog.txt", "a") as file:
            # writes to the file
            file.write(log_entry)
        # takes the whitespaces away from the string to insure its formated right
        if DEBUG:
...
while repeat:
    try:
        # wait
        sleep(30)
        # calls the functing to add the data to the new file evey time the sensors
        # is refreshed.
        temperatureLog().log_temp_file()
